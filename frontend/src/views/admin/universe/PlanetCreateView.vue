<template>
  <AdminLayout title="Crear Nuevo Planeta">
...
const { createPlanet, fetchAllStars } = usePlanets();
...
const loadStars = async () => {
  try {
    ...
    // Si hay estrellas, seleccionar la primera por defecto
    if (stars.value.length > 0) {
      form.star_id = stars.value[0].id.toString();
    }
  } catch (error) {
...
const submitForm = async () => {
  ...
  try {
    const result = await createPlanet(form);
