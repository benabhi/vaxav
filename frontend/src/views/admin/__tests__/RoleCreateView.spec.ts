...
import RoleCreateView from '@/views/admin/RoleCreateView.vue';
...
describe('RoleCreateView', () => {
  ...
  beforeEach(() => {
    ...
    // Mock API response for permissions
  ...
  it('should render the form', () => {
    ...
    expect(wrapper.find('h2').text()).toBe('Crear Nuevo Rol');
  });
  ...
  it('should create role when form is submitted', async () => {
    // Mock API response for role creation
    ...
    expect(api.post).toHaveBeenCalledWith('/admin/roles', expect.objectContaining({
      name: 'Test Role',
      slug: 'test-role',
      description: 'Test role description',
      permissions: [1, 2]
  ...
  it('should auto-generate slug from name', async () => {
    // Set a name
    useForm().values.name = 'Test Role';

    // Trigger the watch function
    await wrapper.vm.$nextTick();

    // Check if slug was generated
    expect(useForm().values.slug).toBe('test-role');
  });
});
