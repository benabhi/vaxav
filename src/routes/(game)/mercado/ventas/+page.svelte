<!--
	Pestaña «Órdenes de venta»: lo que el piloto tiene ofrecido.

	La tabla la pone `OwnOrders`, que es la misma de la pestaña de compras: lo
	único que cambia entre las dos es qué órdenes le llegan.
-->
<script lang="ts">
...
<svelte:head><title>Órdenes de venta · Vaxav</title></svelte:head>

<div class="flex w-full flex-col items-start gap-1">
	<Eyebrow>Lo que ofrecés</Eyebrow>
	<DisplayTitle>Órdenes de venta</DisplayTitle>
</div>
...
<TitledPanel title="Órdenes de venta" detail="{orders.orders.length} abiertas" class="w-full">
	<OwnOrders
		orders={orders.orders}
		empty="No tenés nada publicado a la venta. Se pone desde el mercado."
	/>
</TitledPanel>
...
{#if orders.otherCount > 0}
	...
	<div class="flex w-full items-center gap-3 border-l-[3px] border-l-border bg-surface px-4 py-3">
		...
		<span class="text-1 text-text-muted">
			...
			{orders.otherCount === 1 ? 'orden de compra' : 'órdenes de compra'} puestas.
		</span>
	</div>
{/if}
