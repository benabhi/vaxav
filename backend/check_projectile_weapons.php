...
// Obtener la habilidad de Armas de Proyectil Básico
$skill = $pilot->skills()->where('name', 'Armas de Proyectil Básico')->first();

if (!$skill) {
    // Intentar buscar por nombre parcial
    $skill = $pilot->skills()->where('name', 'like', '%Proyectil%')->first();
    
    if (!$skill) {
        echo "Habilidad 'Armas de Proyectil Básico' no encontrada.\n";
        
        // Listar todas las habilidades del piloto
        echo "\nHabilidades disponibles:\n";
        foreach ($pilot->skills as $s) {
            echo "- " . $s->name . "\n";
        }
...
echo "Información de la habilidad '{$skill->name}':\n";
echo "=================================================================\n";
